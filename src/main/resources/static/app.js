...
// Helper to get selected API version prefix
function getVersionPrefix() {
    const versionElem = document.getElementById('version-v2');
    return versionElem && versionElem.checked ? '/v2' : '/v1';
}
...
async function pingAPI() {
    ...
    try {
        const versionPath = getVersionPrefix();
...
async function apiCall(endpoint, method = 'GET', body = null, customHeaders = {}) {
    ...
    const headers = {
        ...
        'X-API-Version': getVersionPrefix().replace('/v',''),
        ...customHeaders
    };
...
async function handleClientSubmit(e) {
    ...
    try {
        if (id) {
            // Edit
            await apiCall(`${getVersionPrefix()}/clientes/${id}`, 'PUT', payload);
            ...
            await apiCall(`${getVersionPrefix()}/clientes`, 'POST', payload, {
...
async function deleteCliente(e, id) {
    ...
    try {
        await apiCall(`${getVersionPrefix()}/clientes/${id}`, 'DELETE');
...
async function loadEnderecos(clientId) {
    try {
        ...
        const data = await apiCall(`${getVersionPrefix()}/enderecos?size=50`);
...
async function handleAddressSubmit(e) {
    ...
    try {
        if (id) {
            await apiCall(`${getVersionPrefix()}/enderecos/${id}`, 'PUT', payload);
            showToast('Endereço Atualizado', 'Modificações salvas com sucesso.');
        } else {
            await apiCall(`${getVersionPrefix()}/enderecos`, 'POST', payload);
...
async function deleteEndereco(e, id) {
    ...
    try {
        await apiCall(`${getVersionPrefix()}/enderecos/${id}`, 'DELETE');
...
async function loadProdutos() {
    ...
    const versionPrefix = getVersionPrefix();
    let endpoint = `${versionPrefix}/produtos?page=${page}&size=${size}`;
...
async function handleProductSubmit(e) {
    ...
    try {
        if (id) {
            await apiCall(`${getVersionPrefix()}/produtos/${id}`, 'PUT', payload);
            showToast('Produto Atualizado', `${nome} atualizado no cardápio.`);
        } else {
            await apiCall(`${getVersionPrefix()}/produtos`, 'POST', payload);
...
async function deleteProduto(e, id) {
    ...
    try {
        await apiCall(`${getVersionPrefix()}/produtos/${id}`, 'DELETE');
...
async function handleIngredientSubmit(e) {
    ...
    try {
        if (id) {
            await apiCall(`${getVersionPrefix()}/ingredientes/${id}`, 'PUT', payload);
            showToast('Ingrediente Atualizado', 'Alterações salvas.');
        } else {
            await apiCall(`${getVersionPrefix()}/ingredientes`, 'POST', payload);
...
async function deleteIngrediente(e, id) {
    ...
    try {
        await apiCall(`${getVersionPrefix()}/ingredientes/${id}`, 'DELETE');
...
async function loadCartInterface() {
    ...
    try {
        const clientsRes = await apiCall(`${getVersionPrefix()}/clientes?size=100`);
    ...
    try {
        const prodRes = await apiCall(`${getVersionPrefix()}/produtos?size=100`, 'GET');
...
async function handleSubmitOrder() {
    ...
    try {
        const orderCreated = await apiCall(`${getVersionPrefix()}/pedidos`, 'POST', payload, {
            'X-Idempotency-Key': idempotencyKey
        });
...
async function updatePedidoStatus(id, newStatus) {
    try {
        await apiCall(`${getVersionPrefix()}/pedidos/${id}/status?status=${newStatus}`, 'PUT');
...
async function cancelarPedido(id) {
    ...
    try {
        await apiCall(`${getVersionPrefix()}/pedidos/${id}`, 'DELETE');
...
async function loadKeysList() {
    try {
        const keys = await apiCall(`${getVersionPrefix()}/auth/keys`);
...
async function handleGenerateApiKey(e) {
    ...
    try {
        const keyData = await apiCall(`${getVersionPrefix()}/auth/keys?dono=${encodeURIComponent(dono)}`, 'POST');
